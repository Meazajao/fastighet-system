...
export default async function AdminUsersPage() {
  ...
  >
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-[13px] md:text-[14px] font-semibold text-text-primary m-0 mb-0.5">{u.name}</p>
        <p className="text-[11px] md:text-[12px] text-text-muted m-0 hidden sm:block">
          {u.email}{u.apartment ? ` · Lgh ${u.apartment}` : ""}
        </p>
        <p className="text-[11px] text-text-muted m-0 sm:hidden">{u.email}</p>
      </div>
    </div>

    <div className="flex items-center gap-2 md:gap-4">
      <span className="hidden md:block text-[12px] text-text-muted">
        {u._count.tickets} ärenden
      </span>
      <span
        className="text-[10px] md:text-[11px] font-semibold px-2 md:px-3 py-1 rounded-full"
        style={{
          background: u.role === "ADMIN" ? "#f3f0ff" : "#f5f5f7",
          color: u.role === "ADMIN" ? "#5e35b1" : "#6e6e73",
        }}
      >
        {u.role === "ADMIN" ? "Admin" : "Hyresgäst"}
      </span>
    </div>
  </div>
))}
